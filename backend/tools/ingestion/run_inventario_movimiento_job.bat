...
set "LOG_FILE=%LOG_DIR%\inventario_movimiento_job.log"
set "STATUS_FILE=%STATUS_DIR%\inventario_movimiento_job_last_status.txt"
set "LOCK_FILE=%STATUS_DIR%\inventario_movimiento_job.lock"

REM ============================================
REM 3) INICIAR LOG LIMPIO
REM ============================================
> "%LOG_FILE%" echo [%DATE% %TIME%] INICIO LOG inventario_movimiento_job
...
if not exist "%PROJECT_ROOT%\backend\tools\ingestion\inventario_movimiento_job.py" (
    echo [%DATE% %TIME%] ERROR: no existe inventario_movimiento_job.py >> "%LOG_FILE%"
...
REM 8) EJECUTAR JOB
REM ============================================
echo [%DATE% %TIME%] INICIO python -m backend.tools.ingestion.inventario_movimiento_job >> "%LOG_FILE%"

"%PYTHON_EXE%" -m backend.tools.ingestion.inventario_movimiento_job >> "%LOG_FILE%" 2>&1
set "RC=%ERRORLEVEL%"
...
REM ============================================
if %RC% NEQ 0 (
    echo [%DATE% %TIME%] ERROR inventario_movimiento_job rc=%RC% log=%LOG_FILE% >> "%LOG_FILE%"
    > "%STATUS_FILE%" echo FAILED_%RC%
    del "%LOCK_FILE%" >nul 2>&1
    exit /b %RC%
) else (
    echo [%DATE% %TIME%] OK inventario_movimiento_job log=%LOG_FILE% >> "%LOG_FILE%"

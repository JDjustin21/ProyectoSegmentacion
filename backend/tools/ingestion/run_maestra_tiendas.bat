...
set "LOG_FILE=%LOG_DIR%\maestra_tiendas.log"
set "STATUS_FILE=%STATUS_DIR%\maestra_tiendas_last_status.txt"
set "LOCK_FILE=%STATUS_DIR%\maestra_tiendas.lock"

REM ============================================
REM 3) TIMESTAMP PARA LOG
REM ============================================
> "%LOG_FILE%" echo [%DATE% %TIME%] INICIO LOG maestra_tiendas
...
REM ============================================
echo [%DATE% %TIME%] INICIO maestra_tiendas_job.py >> "%LOG_FILE%"
...
REM ============================================
if %RC% NEQ 0 (
    echo [%DATE% %TIME%] ERROR maestra_tiendas_job rc=%RC% log=%LOG_FILE% >> "%LOG_FILE%"
    > "%STATUS_FILE%" echo FAILED_%RC%
    del "%LOCK_FILE%" >nul 2>&1
    exit /b %RC%
) else (
    echo [%DATE% %TIME%] OK maestra_tiendas_job log=%LOG_FILE% >> "%LOG_FILE%"
